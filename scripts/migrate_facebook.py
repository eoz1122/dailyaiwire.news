"""
Migration: Add shared_on_fb column to articles table.
Tracks whether an article has been shared on Facebook.
"""
import sqlite3
import os
...
def migrate():
    ...
    if "shared_on_fb" in columns:
        print("✅ Column 'shared_on_fb' already exists. Nothing to do.")
        conn.close()
        return

    cursor.execute("ALTER TABLE articles ADD COLUMN shared_on_fb BOOLEAN DEFAULT 0")
    conn.commit()
    print("✅ Added 'shared_on_fb' column to articles table.")
    conn.close()
